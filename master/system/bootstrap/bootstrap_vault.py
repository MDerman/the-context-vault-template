...
GENERATED_MARKER = "managed-by: master/system/bootstrap/bootstrap_vault.py"
MASTER_PERIODIC_MARKER = "managed-by: master/system/scripts/periodic.py"
OLD_GENERATED_MARKERS = (
    "managed-by: master/system/bootstrap/setup/bootstrap_vault.py",
    "managed-by: master/bootstrap/setup/bootstrap_vault.py",
)
OLD_MASTER_PERIODIC_MARKERS = (
    "managed-by: master/system/scripts/generate_master_periodic_notes_for_now.py",
    "managed-by: master/scripts/generate_master_periodic_notes_for_now.py",
)
...
class Bootstrap:
    ...
    def write_entity_home(self, entity: str, status: str, content_enabled: bool, default_capture: bool) -> None:
        path = self.root / entity / "HOME.md"
        content = entity_home(status, content_enabled, default_capture)
        if not path.exists():
            self.ensure_dir(path.parent)
            self.log(f"write {rel(path, self.root)}")
            if not self.dry_run:
        ...
        if not any(marker in existing for marker in BOOTSTRAP_MARKERS) and not has_frontmatter(existing):
            self.log(f"skip existing non-managed file {rel(path, self.root)}")
            return
        content = update_entity_home(existing, status, content_enabled, default_capture)
        if existing == content:
            return
        self.log(f"write {rel(path, self.root)}")
        if not self.dry_run:
    ...
    def setup_directories(self) -> None:
        master_dirs = [
            "master/_obsidian/bases",
            "master/_obsidian/bases",
            "master/system/context",
            "master/_obsidian/notes/operating-methods",
            "master/_obsidian/templates/shared/content",
    ...
    def cleanup_retired_monthly_periodics(self) -> None:
        ...
        self.safe_remove_generated_path(self.root / "master/_obsidian/periodic/monthly", MANAGED_MARKERS)
        for entity in self.entities:
    ...
    def setup_entity_homes(self) -> None:
        ...
        for entity in self.entities:
            ...
            self.write_entity_home(entity, status, entity in content, entity == self.default_entity)

    def setup_agent_infrastructure(self) -> None:
        if self.coding_agents:
            self.ensure_dir(self.root / "master/agents/skills")
            self.ensure_dir(self.root / "master/agents/skills-dump")
    ...
    def setup_excalidraw(self) -> None:
        master_excalidraw = self.root / "master/_obsidian/excalidraw"
    ...
    def setup_templates(self) -> None:
        ...
        self.write_managed(
            self.root / "master/README_PERSONALIZED_QUICKSTART.md",
            personalized_quickstart(self.entities, self.default_entity, self.active_entities, self.content_entities),
        )
        self.write_managed(self.root / "master/_obsidian/templates/shared/default-tasks-template.md", shared_task_template())
        self.write_managed(self.root / "master/_obsidian/templates/shared/content/content-item-template.md", content_item_template())
        self.write_managed(self.root / "master/_obsidian/templates/shared/content/publication-template.md", publication_template())
        self.write_managed(self.root / "master/_obsidian/templates/shared/declarations/personal.md", declaration_template("01-personal", "personal"))
        self.write_managed(self.root / "master/_obsidian/templates/shared/declarations/personal-brand.md", declaration_template("02-personal-brand", "personal-brand"))
        self.write_managed(self.root / "master/_obsidian/templates/shared/declarations/company.md", declaration_template("03-business", "company"))
    ...
    def setup_bases(self) -> None:
        ...
        for name, content in master_bases.items():
            self.write_managed(
                self.root / "master/_obsidian/bases" / name,
                content,
                allow_unmarked_existing=True,
            )

        self.remove_path(self.root / "master/_obsidian/bases/Master Dashboard.base")
        self.remove_path(self.root / "master/_obsidian/bases/All Tasks.base")
        for old_name in [
            "All Epics.base",
            "Today.base",
            "This Week.base",
            ...
            self.remove_path(self.root / "master/_obsidian/bases" / old_name)
        self.remove_empty_dir(self.root / "master/tasknotes")
        master_task_views = self.root / "master/_obsidian/bases"
        task_view_names = {
            "tasks-agenda.base",
            "tasks-calendar.base",
            "tasks-kanban.base",
    ...
    def install_vault_command(self) -> None:
        ...
        if not self.install_vault_command_enabled:
            ...
            return
        installer_path = self.root / "master/system/bootstrap/install_vault_command.py"
    ...
    def generate_agents_file(self) -> None:
        ...
        if not self.generate_agents_enabled or not self.coding_agents:
            ...
            return
        generator_path = self.root / "master/system/bootstrap/generate_agents.py"
    ...
    def run_generators(self) -> None:
        ...
        periodic_path = self.root / "master/system/scripts/periodic.py"
        agent_path = self.root / "master/system/scripts/context.py"
        if self.dry_run:
...
def entity_home(status: str, content_enabled: bool = False, default_capture: bool = False) -> str:
...
def update_entity_home(existing: str, status: str, content_enabled: bool, default_capture: bool) -> str:
    ...
    for line in lines:
        if ":" in line and not line.startswith(" "):
            ...
            if key in desired:
                ...
                continue
        output.append(line)
    for key in ["status", "content_enabled", "default_capture"]:
        if key not in seen:
            output.append(f"{key}: {desired[key]}")
    return "---\n" + "\n".join(output) + "\n---\n" + body
...
- `master`: operating manual, generated dashboards, setup scripts, shared templates, agent context, agent skills, reusable scripts, media, and Mac/dev tools.
- `library`: learning material, research, swipe files, downloaded examples, lead magnets, and source material.
- `wiki`: synthesized knowledge built by LLMs using `wiki/AGENTS.md` and `wiki/karpathy-initial-proompt.md`.
...
`DECLARATION.md` is the entity's durable operating source for Identity and Momentum. For personal-brand entities, Social Selling lives as a third-level section inside Momentum. Each context folder owns its local periodic templates under `_obsidian/templates/periodic`. `master/_obsidian/templates/shared` is for root-level shared non-periodic templates, declaration templates, content templates, and the default TaskNotes template.
...
- `master/system/context/CONTEXT.md` and `master/system/context/context.json`: generated current state for agents.
- `master/system/context/*.md`: generated readable files for agents plus durable agent operating notes.
...
4. Open the current flat rollup in `master/system/context/` for readable rollups across active context folders.
5. Open `master/_obsidian/bases` and `master/_obsidian/bases` for dashboards and task views.

Task routing:

```text
No context or default context -> {default_entity}/_obsidian/tasks
@03-business              -> 03-business/_obsidian/tasks
@04-dev                     -> 04-dev/_obsidian/tasks
```
...
Agent rollups:       master/system/context/<period-id>.md
```
...
Context folder periodic notes remain the editable source of truth. Sync Embeds notes live at `master/system/obsidian_notes/beta_plugins_docs/README-sync-embeds.md`.
...
- Active context folder operating material: the matching numbered context folder.
...
- Learning, research, downloaded templates, lead magnets: `library`.
- Durable synthesized knowledge: `wiki`.
- Reusable assets/scripts/media: `master`.
- Old or uncertain material: `other`.

Context folders are not learning folders. If learning from `library` or `wiki` belongs in a context folder, rewrite it as an operating artifact first: SOP, keep-in-mind note, training note, checklist, playbook, decision record, policy, or active reference.
...
```text
Prepare launch checklist tomorrow @03-business #marketing high
Review contract next Friday @04-dev +[[Client Work]] in-progress
Draft weekly reflection @01-personal 30min
```
...
- `master/00-StartHere.md`
- `master/01-Context.md`
- `master/system/context/<today>.md`
- `master/_obsidian/bases/content-kanban.base`
- `master/_obsidian/bases/tasks-home.base`
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--default-context-folder", dest="default_entity", metavar="CONTEXT_FOLDER", default="01-personal", help="Default capture context folder.")
