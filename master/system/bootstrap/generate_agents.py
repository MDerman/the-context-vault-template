...
MARKER = "managed-by: master/system/bootstrap/generate_agents.py"
OLD_MARKERS = (
    "managed-by: master/system/bootstrap/bootstrap_vault.py",
    "managed-by: master/system/bootstrap/setup/bootstrap_vault.py",
    "managed-by: master/bootstrap/setup/bootstrap_vault.py",
)
TEMPLATE_PATH = Path("master/system/bootstrap/AGENTS.template.md")
...
def ensure_agent_symlinks(root: Path, dry_run: bool) -> None:
    skills_source = root / "master/agents/skills"
    for directory in [root / "master/agents/skills", root / "master/agents/skills-dump", root / ".agents", root / ".claude"]:
        if directory.exists():
            continue
        if dry_run:
    ...
    ensure_symlink(root, Path(".agents/skills"), "../master/agents/skills", dry_run, directory=True)
