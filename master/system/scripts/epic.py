...
MARKER = "managed-by: master/system/scripts/epic.py"
...
def parse_args() -> argparse.Namespace:
    ...
    create_parser.add_argument("context_folder", help="Context folder, e.g. 03-business.")
    ...
    delete_parser.add_argument("context_folder", help="Context folder, e.g. 03-business.")
    ...
    rename_parser.add_argument("context_folder", help="Context folder, e.g. 03-business.")
...
def context_label(context_folder: str) -> str:
    match = re.match(r"^\d\d-(.+)$", context_folder)
    label = match.group(1) if match else context_folder
...
def update_master_kanban_views(root: Path, context_roots: list[Path], dry_run: bool) -> str:
    path = root / "master/_obsidian/bases/tasks-kanban-v1.base"
    if not path.exists():
        return "missing"
...
def run_epic_base_generator(root: Path, context_folders: str | None, include_archived: bool, dry_run: bool) -> int:
    script = root / "master/system/scripts/generate_epic_kanban_views.py"
...
def sync(root: Path, context_folders: str | None, include_archived: bool, dry_run: bool) -> int:
    ...
    print(f"master tasks-kanban-v1 epic views: {result}: master/_obsidian/bases/tasks-kanban-v1.base")
    return 0
