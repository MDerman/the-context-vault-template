...
BLOG_SOURCE = Path("02-personal-brand/Matt writing etc/Blogs/Blog Database")
YOUTUBE_SOURCE = Path("02-personal-brand/Matt writing etc/Youtube Factory/Content Database")
DEFAULT_MANIFEST = Path("master/migration-reports/matt-content-migration-manifest.json")
ID_TAIL = re.compile(
...
def destination_for_blog(root: Path, source_root: Path, path: Path, title: str, metadata: dict[str, str]) -> tuple[str, Path, str]:
    ...
    entity = "03-business" if "impression" in website.lower() else "02-personal-brand"
    publication = "impression" if entity == "03-business" else "matt-derman"
...
def destination_for_youtube(root: Path, source_root: Path, path: Path, title: str, metadata: dict[str, str]) -> tuple[Path, str, str]:
    ...
    if len(relative.parts) > 1:
        parent_slug = slugify(relative.parts[0])
        filename = relative_slug(Path(*relative.parts[1:])) + ".md"
        destination = root / "02-personal-brand" / "_obsidian/content/items" / folder / parent_slug / filename
    else:
        destination = root / "02-personal-brand" / "_obsidian/content/items" / folder / f"{slugify(title)}.md"
    ...
    for path in sorted(source_root.rglob("*.md")):
        ...
        items.append(
            MigrationItem(
                ...
                content_kind=kind,
                platform=platform,
                publication="matt-derman-youtube" if platform == "youtube" else metadata.get("Platform", ""),
