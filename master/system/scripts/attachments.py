...
IMPORT_FOLDERS = [
    ROOT / "02-personal-brand" / "_obsidian" / "attachments" / "notion-import",
    ROOT / "03-business" / "_obsidian" / "attachments" / "notion-task-import",
]
IMPORT_MARKERS = {
    "notion-import": ROOT / "02-personal-brand" / "_obsidian" / "attachments" / "notion-import",
    "notion-task-import": ROOT / "03-business" / "_obsidian" / "attachments" / "notion-task-import",
}
...
def should_migrate_reference(note: Path, source: Path) -> tuple[bool, str, bool]:
    ...
    if is_under(source, MASTER_ATTACHMENTS) and "_inbox" not in source.relative_to(MASTER_ATTACHMENTS).parts:
        if note_root != "master":
            return True, "master-attachment-cross-root-reference", False
        return False, "", False
...
def verify() -> int:
    ...
    for note in iter_markdown_files():
        ...
        for span in iter_link_spans(text):
            ...
            if note_root != "master" and is_under(source, MASTER_ATTACHMENTS):
                failures.append(f"{rel(note)} links to master attachment {rel(source)}")
