...
CONTEXT_FOLDER_NAME = re.compile(r"^\d{2}-")
...
def write_home(path: Path, status: str, content_enabled: bool) -> None:
...
def discover_entities(root: Path) -> list[str]:
    ...
    for path in sorted(root.iterdir()):
        if not path.is_dir() or path.name.startswith("."):
            continue
        if CONTEXT_FOLDER_NAME.match(path.name) and (path / "HOME.md").exists():
...
def load_bootstrap(root: Path):
    path = root / "master/system/bootstrap/bootstrap_vault.py"
    if not path.exists():
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("-n", "--name", required=True, help="Context folder name, for example 06-new-context-folder.")
    parser.add_argument(
        "-s",
        "--status",
        required=True,
    ...
    parser.add_argument("--default-context-folder", dest="default_entity", metavar="CONTEXT_FOLDER", default=None, help="Default capture context folder. Defaults to current root TaskNotes setting or 01-personal.")
    ...
    if args.dry_run:
        ...
        write_home(entity_root / "HOME.md", args.status, args.content_enabled)
    ...
    default_entity = args.default_entity or "01-personal"
