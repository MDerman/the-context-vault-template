...
- The root vault attachment folder is configured as the temporary inbox `master/_obsidian/attachments/_inbox`.
- The vault convention is to route note attachments to the owning top-level folder's `_obsidian/attachments` directory, such as `library/_obsidian/attachments`.
