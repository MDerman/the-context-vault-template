...
```text
master/_obsidian/attachments/_inbox
```
...
```json
{ "path": "02-personal-brand/Youtube Factory", "color": "<palette-id>" }
```

Reuse existing palette IDs by preset name instead of hardcoding new colors. Root `_obsidian` folders and `master` should use the Obsidian purple preset. `02-personal-brand/Youtube Factory` should use the YouTube red preset.
...
```text
master/_obsidian/templates/shared/
```
...
Agent periodic rollups under `master/system/context` are generated as plain readable Markdown so agents do not need Obsidian plugin rendering.
