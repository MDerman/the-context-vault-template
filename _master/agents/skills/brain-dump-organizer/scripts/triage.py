...
INBOX_PATH = Path("_master/system/inbox/BRAIN_DUMP.md")
ATTACHMENTS_DIR = Path("_master/system/inbox/BRAIN_DUMP_ATTACHMENTS")
BACKUPS_DIR = Path("_master/system/inbox/BRAIN_DUMP_BACKUPS")
PROPOSALS_DIR = Path("_master/system/inbox/BRAIN_DUMP_PROPOSALS")
BASE_PATH = Path("_master/_obsidian/bases/BRAIN_DUMP_TRIAGE.base")
...
ROUTE_TARGETS = {
    "personal-task": {
        "context": "personal",
        "kind": "task",
        "directory": Path("personal/_obsidian/tasks"),
    },
    "matt-task": {
    ...
    "library-thought": {
        "context": "_library",
        "kind": "library-note",
        "directory": Path("_library/thoughts"),
    },
}
...
def resolve_root(root_arg: str | None = None) -> Path:
    ...
    for candidate in [current, *current.parents]:
        if (candidate / "AGENTS.md").exists() and (candidate / "_master/system").is_dir():
            return candidate
...
def destination_attachment_dir(route: str, target_path: Path, run_id: str) -> Path:
    if route in ROUTE_TARGETS:
        ...
        if context in {"personal", "personal-brand", "business"}:
            return Path(context) / "_obsidian/attachments/brain-dump" / run_id
        if context == "_library":
            return Path("_library/_attachments/brain-dump") / run_id
    if target_path.parts and target_path.parts[0] in {"personal", "personal-brand", "business"}:
...
def cmd_self_test(_: argparse.Namespace) -> int:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        (root / "_master/system").mkdir(parents=True, exist_ok=True)
