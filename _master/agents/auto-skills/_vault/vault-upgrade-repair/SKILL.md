...
description: Repair public bootstrap vault upgrades after `vault upgrade` reports skipped files, failed migrations, or conflicts. Use when user mentions vault upgrade repair, `.vault-upgrade/report.json`, failed migrations, or asks an agent to fix upgrade fallout.
---
...
```bash
ls -td .vault-upgrade/* | head -1
sed -n '1,220p' .vault-upgrade/<timestamp>/report.json
```

2. Inspect only paths named in report, migration reports, and relevant policy:

```bash
sed -n '1,220p' .vault-bootstrap/policy.json
```
...
vault refresh --skip-brain-dump --skip-gcal --skip-git-maintenance
```
