...
- `refresh.py`: runs the vault refresh pipeline; by default it runs the Google Calendar task mirror, regenerates agent context, updates `_master/Dashboard.md`, and removes stale generated non-monthly agent periodic rollups unless `--keep-agent-periodic-history` is passed. Use `--sync-brain-dump` to also ingest the Brain Dump Apple Note.
