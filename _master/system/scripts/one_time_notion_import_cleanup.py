...
WIKI_MASTER_ATTACHMENT_RE = re.compile(
    r"\[\[_master/_obsidian/attachments/([^\]|#]+)(#[^\]|]+)?(\|[^\]]+)?\]\]"
)
LOCAL_SKIP_SCHEMES = (
    "http://",
    "https://",
...
def learning_frontmatter(source_path: Path, dest_path: Path, metadata: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if not parts or parts[0] != "Learning" or dest_path.suffix.lower() != ".md":
        return None
    props: dict[str, Any] = {"contexts": ["personal-brand"]}
...
def update_iconize(dry_run: bool) -> Counter:
    stats: Counter = Counter()
    updates = {
        "personal-brand/Matt writing etc": "✍️",
        "personal-brand/Matt writing etc.md": "✍️",
        "personal-brand/Matt writing etc/Blogs": "📝",
        "personal-brand/Matt writing etc/Youtube Factory": "🎬",
        "personal-brand/Matt writing etc/Content Schedules": "🗓️",
        "personal-brand/Matt writing etc/Content Pillars.md": "🏛️",
        "personal-brand/Learning": "🎓",
        "personal-brand/Learning/Dev": "💻",
        "personal-brand/Learning/UCT - Student Dashboard": "🎓",
    }
...
def verify() -> int:
    ...
    if id_paths:
        failures.append(f"personal-brand still has {len(id_paths)} visible paths ending in Notion IDs")
