...
```bash
sed -n '1,60p' "{{sample_task_path}}"
```

Common queries:

```bash
rg -l '^\s*epic:.*Current dev' {{sample_task_dir}}
rg -l '^status: in-progress$' {{active_task_dirs}} | head -5
for s in in-progress ongoing to-be-resumed up-next backlog; do rg -l "^status: $s$" {{active_task_dirs}}; done | head -50
