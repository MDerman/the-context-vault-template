...
Problem: Obsidian's built-in attachment setting cannot route pasted files to the top-level root folder that owns the current note. The current workaround is to paste into `_master/_obsidian/attachments/_inbox` and periodically run:

```bash
python3 _master/system/scripts/attachments.py --apply
```
...
- Watch `_master/_obsidian/attachments/_inbox` for new files.
