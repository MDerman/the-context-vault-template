...
description: Routes work involving multiple computers, primary/worker vault coordination, remote access, Mac or Linux setup, code locations, repository placement, and fleet-wide agent deployment. Use before connecting to, configuring, enrolling, recovering, or locating code on any machine.
---

# Infra · Code Folder and Computer Topology

Read [[_system/agents/auto-skills/_infrastructure/infra-code-folder-and-computer-topology/README-primary-worker-vault-sync|Primary and Worker Vault Coordination]] before acting. It defines generic fleet roles, registry routing, checkout rules, and safe Git behavior.

- Read `_system/config/infra-code-folder-and-computer-topology/README.md`, then `private/machines.json` and `private/repositories.json` when present. Missing registry means fleet automation stays inactive; show `vault machine init` guidance.
