...
// src/periodic-tab-rollover-service.ts
var import_obsidian9 = require("obsidian");

// src/periodic-tab-rollover.ts
var REFRESH_COMPLETION_MARKER_PATH = "_system/state/refresh-complete.json";
var PERIOD_KINDS = ["daily", "weekly", "monthly", "quarterly", "yearly"];
var PERIODIC_NOTE_PATH_RE = /^(?<scope>[^/]+)\/_obsidian\/periodic\/(?<kind>daily|weekly|monthly|quarterly|yearly)\/(?<id>[^/]+)\.md$/;
...
function activePeriodsForDate(date) {
  const year = date.getFullYear();
...
function parsePeriodicNotePath(path) {
...
function rolloverTargetPath(path, activePeriods) {
...
function isPastPeriod(kind, candidateId, currentId) {
...
function parseRefreshCompletionPayload(value) {
  if (!isRecord2(value) || value.schemaVersion !== 1) {
    return null;
  }
  if (typeof value.runId !== "string" || !value.runId || typeof value.completedAt !== "string" || typeof value.effectiveDate !== "string" || !isRecord2(value.periods)) {
    return null;
  }
...
function periodSortKey(kind, value) {
  if (kind === "daily") {
  ...
  if (kind === "weekly") {
    const match2 = /^(\d{4})-W(\d{2})$/.exec(value);
    if (!match2) {
      return null;
    }
    const year = Number(match2[1]);
    const week = Number(match2[2]);
  ...
  if (kind === "monthly") {
    const match2 = /^(\d{4})-(\d{2})$/.exec(value);
    if (!match2) {
      return null;
    }
    const year = Number(match2[1]);
  ...
  if (kind === "quarterly") {
    const match2 = /^(\d{4})-Q([1-4])$/.exec(value);
    return match2 ? [Number(match2[1]), Number(match2[2])] : null;
  }
...
function isoWeeksInYear(year) {
  ...
  const yearStart = new Date(Date.UTC(year, 0, 1));
  return Math.ceil(((december28.getTime() - yearStart.getTime()) / 864e5 + 1) / 7);
}
...
// src/periodic-tab-rollover-service.ts
var PeriodicTabRolloverService = class {
  constructor(app, options = {}) {
    this.app = app;
    this.lastHandledRunId = null;
    this.activeRollover = null;
    var _a, _b, _c, _d;
    this.now = (_a = options.now) != null ? _a : (() => /* @__PURE__ */ new Date());
    this.retryAttempts = (_b = options.retryAttempts) != null ? _b : 10;
    this.retryDelayMs = (_c = options.retryDelayMs) != null ? _c : 500;
    this.markerPollMs = (_d = options.markerPollMs) != null ? _d : 6e4;
  }
  register(plugin) {
    this.app.workspace.onLayoutReady(() => {
  ...
  async rollForwardToToday(source = "manual") {
  ...
  async readCompletionMarker() {
    try {
      ...
      return parseRefreshCompletionPayload(JSON.parse(raw));
    } catch (error) {
      console.warn("[Context Nine] Could not read vault refresh completion marker", error);
      return null;
    }
  }
  async performRollover(source) {
    var _a;
    ...
    if (updated > 0) {
      new import_obsidian9.Notice(`Context Nine updated ${updated} past periodic tab${updated === 1 ? "" : "s"}.`);
...
function filePathFromLeaf(leaf) {
  var _a;
  const file = (_a = leaf.getViewState().state) == null ? void 0 : _a.file;
...
var ContextNinePlugin = class extends import_obsidian10.Plugin {
  ...
  openVaultTui() {
    ...
    if (!cwd) {
      new import_obsidian10.Notice("Vault root is not configured.");
      return;
    }
    ...
    child.on("error", (error) => {
      new import_obsidian10.Notice(`Could not open vault TUI: ${error.message}`);
    });
    child.on("close", (exitCode) => {
      if (exitCode !== 0) {
        new import_obsidian10.Notice(`Could not open vault TUI: osascript exited ${exitCode}`);
      }
    });
  }
  ...
  async getVaultCockpitView() {
    ...
    if (!leaf) {
      new import_obsidian10.Notice("Could not open the right sidebar.");
      return null;
    }
  ...
  async runVaultCommandInCockpit(command, extraArgs = []) {
    ...
    if (!view) {
      new import_obsidian10.Notice("Could not open the vault command center.");
      return;
    }
  ...
  async focusMainPane(index, createIfMissing = false) {
    ...
    if (!leaf) {
      new import_obsidian10.Notice(`Could not find main pane ${index + 1}.`);
      return;
    }
    await this.app.workspace.revealLeaf(leaf);
  ...
  async focusRightSidebar() {
    ...
    if (!leaf) {
      new import_obsidian10.Notice("Could not focus the right sidebar.");
      return;
    }
    await this.app.workspace.revealLeaf(leaf);
    this.app.workspace.setActiveLeaf(leaf, { focus: true });
  ...
  registerAttachmentWatcher() {
    this.registerEvent(
      this.app.vault.on("create", (file) => {
        if (file instanceof import_obsidian10.TFile && this.router.isInboxPath(file.path)) {
  ...
  async routeAttachmentInboxNow() {
    const activeFile = this.app.workspace.getActiveFile();
    if (!activeFile) {
      new import_obsidian10.Notice("Open the note that owns the pasted attachment, then run this command again.");
      return;
    }
...
var VaultPromptArgsModal = class extends import_obsidian10.Modal {
  ...
  onOpen() {
    ...
    promptArgs.forEach((promptArg, index) => {
      new import_obsidian10.Setting(this.contentEl).setName(promptArg.label).addText((text) => {
        var _a2;
  ...
  submit() {
    ...
    if (missing) {
      new import_obsidian10.Notice(`${missing.label} required.`);
      return;
    }
    this.close();
...
var ContextNineSettingTab = class extends import_obsidian10.PluginSettingTab {
  constructor(plugin) {
  ...
  display() {
    ...
    new import_obsidian10.Setting(containerEl).setName("Attachment inbox path").addText((text) => {
    ...
    new import_obsidian10.Setting(containerEl).setName("Auto-route attachment inbox").addToggle((toggle) => {
    ...
    new import_obsidian10.Setting(containerEl).setName("Route interval seconds").addText((text) => {
    ...
    new import_obsidian10.Setting(containerEl).setName("Move TaskNotes files when context changes").addToggle((toggle) => {
    ...
    new import_obsidian10.Setting(containerEl).setName("Hovered file actions").addToggle((toggle) => {
    ...
    new import_obsidian10.Setting(containerEl).setName("TaskNotes edit modal enhancements").addToggle((toggle) => {
    ...
    new import_obsidian10.Setting(containerEl).setName("Vault command").addText((text) => {
    ...
    new import_obsidian10.Setting(containerEl).setName("Vault root").addText((text) => {
